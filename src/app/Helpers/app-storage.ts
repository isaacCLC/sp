...
import { CurrentClaim } from './claim-manager';
import { DeviceInfo } from './device-info';
import { getHomeAlertButtonsResponse, getHomeProductListResponse, getSchemeInfoSkin, SubmemberLoginData } from './responses';
...
export class AppStorage {
    ...
    async logoutClear() {
        ...
        await this.setSubMemberData(null);
...
///////
    ...
    ////////////
    public async getSubMemberData(): Promise<SubmemberLoginData> {
        let item = await this.getValue<SubmemberLoginData>("SubMemberLoginData");
        if (!item)
            return null;
        return item;
    };
    public setSubMemberData(value: SubmemberLoginData) {
        console.log("SUb memeber data")
        console.log(value)
        return this.setValue("SubMemberLoginData", value);
    }

    ////////////
    public async getSchemeInfo(): Promise<getSchemeInfoSkin> {
        let item = await this.getValue<getSchemeInfoSkin>("SchemeInfoSkin");
        if (!item)
            return null;
        return item;
    };
    public setSchemeInfo(value: getSchemeInfoSkin) {
        return this.setValue("SchemeInfoSkin", value);
    }

    ////////////
    public async getHomeProductList(): Promise<getHomeProductListResponse> {
        let item = await this.getValue<getHomeProductListResponse>("HomeProductList");
        if (!item)
            return null;
        return item;
    };
    public setHomeProductList(value: getHomeProductListResponse) {
        return this.setValue("HomeProductList", value);
    }

    ////////////
    public async getHomeAlertButtons(): Promise<getHomeAlertButtonsResponse> {
        let item = await this.getValue<getHomeAlertButtonsResponse>("HomeAlertButtons");
        if (!item)
            return null;
        return item;
    };
    public setHomeAlertButtons(value: getHomeAlertButtonsResponse) {
        return this.setValue("HomeAlertButtons", value);
    }
